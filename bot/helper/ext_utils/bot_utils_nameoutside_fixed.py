...
def get_readable_message():
    ...
    for i, download in enumerate(list(download_dict.values())[STATUS_START:STATUS_LIMIT+STATUS_START], start=1):
        # File/Task name OUTSIDE the box (like your screenshot)
        ...
        # Show filename line first (outside the box)
        ...
        # User mention (HTML-safe from pyrogram)
        ...
        # Normal modes (down/up/etc) vs seeding/splitting
    ...
    # Bot stats (compact, like your screenshot)
    msg += "\n<b>⌬ Bot Stats</b>\n"
    msg += f"⚡ CPU » {cpu_percent()}%  |  💾 Free » {get_readable_file_size(disk_usage(config_dict['DOWNLOAD_DIR']).free)} [{round(100-disk_usage(config_dict['DOWNLOAD_DIR']).percent, 1)}%]\n"
    msg += f"🧠 RAM » {virtual_memory().percent}%  |  🕒 UPTIME » {get_readable_time(time() - botStartTime)}\n"

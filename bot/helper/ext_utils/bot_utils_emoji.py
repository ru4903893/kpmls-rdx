...
def get_readable_message():
    ...
    for download in list(download_dict.values())[STATUS_START:STATUS_LIMIT+STATUS_START]:
        ...
        if download.status() not in [MirrorStatus.STATUS_SPLITTING, MirrorStatus.STATUS_SEEDING]:
            ...
            msg += BotTheme('ENGINE', Engine=f"🧩 {download.eng()}")
            msg += BotTheme('STA_MODE', Mode=f"🎛️ {download.upload_details['mode']}")
            ...
            msg += BotTheme('TIME', Time=f"🕒 {download.seeding_time()}")
            msg += BotTheme('SEED_ENGINE', Engine=f"🧩 {download.eng()}")
        else:
            msg += BotTheme('STATUS', Status=download.status(), Url=msg_link)
            msg += BotTheme('STATUS_SIZE', Size=f"📦 {download.size()}")
            msg += BotTheme('NON_ENGINE', Engine=f"🧩 {download.eng()}")

        msg += BotTheme('USER',
                        User=f"👤 {download.message.from_user.mention(style='html')}")
        msg += BotTheme('ID', Id=f"🆔 {download.message.from_user.id}")
    ...
    msg += BotTheme('Cpu', cpu=f"🧠 {cpu_percent()}")
    msg += BotTheme('FREE', free=f"💾 {get_readable_file_size(disk_usage(config_dict['DOWNLOAD_DIR']).free)}", free_p=round(100-disk_usage(config_dict['DOWNLOAD_DIR']).percent, 1))
    msg += BotTheme('Ram', ram=f"🧬 {virtual_memory().percent}")
    msg += BotTheme('uptime', uptime=f"⏱️ {get_readable_time(time() - botStartTime)}")
    msg += BotTheme('DL', DL=f"⬇️ {get_readable_file_size(dl_speed)}")
    msg += BotTheme('UL', UL=f"⬆️ {get_readable_file_size(up_speed)}")
    return msg, button

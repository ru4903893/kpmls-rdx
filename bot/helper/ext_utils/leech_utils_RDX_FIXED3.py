...
async def format_filename(file_, user_id, dirpath=None, isMirror=False):
    ...
    # RDX: remove common junk tags from filename/title (keeps template fields cleaner)
    file_ = re_sub(r'@\w+', '', file_)
    file_ = re_sub(r'(?i)\b(10\s*bit|10bit|8\s*bit|8bit)\b', '', file_)
    file_ = re_sub(r'(?i)\bWEB\s*DL\b', 'WEB-DL', file_)
    file_ = re_sub(r'(?i)\b(vegamovies|skymovieshd|mkvcinemas|hdhub4u|moviesmod|katmoviehd|bollyflix|filmyzilla|toonworld4all|yts|hot)\b', '', file_)

...
from bot.helper.ext_utils.smart_rename import smart_rename
...
async def format_filename(file_, user_id, dirpath=None, isMirror=False):
    ...
    file_ = smart_rename(file_, skip_series=False)
    ...
    cap_mono = f"<{config_dict['CAP_FONT']}>{nfile_}</{config_dict['CAP_FONT']}>" if config_dict.get('CAP_FONT') else nfile_

...
def get_readable_message():
    ...
    for download in list(download_dict.values())[STATUS_START:STATUS_LIMIT + STATUS_START]:
        ...
        pbar = _circle_bar(progress, 12)

        done_txt = download.processed_bytes()
        total_txt = download.size()
        status_txt = download.status()
        engine_txt = download.eng()
        ...
        stop_cmd = f"/{BotCommands.CancelMirror}_{download.gid()}"

        msg += (
            f"<b>╭───────────────</b>\n"
            f"<b>│ Task By 👤</b> {user_mention} <b>( #ID{user_id} )</b>\n"
            f"<b>│</b> {pbar} <b>{progress}</b>\n"
        )

        if status_txt not in [MirrorStatus.STATUS_SPLITTING, MirrorStatus.STATUS_SEEDING]:
            msg += (
                f"<b>│ Done ✔ »</b> {done_txt}\n"
                f"<b>│ Total 😲 »</b> {total_txt}\n"
                f"<b>│ Status 🗿 »</b> <i>{escape(status_txt)}</i>\n"
                f"<b>│ Speed 🚀 »</b> {download.speed()}\n"
                f"<b>│ ETA 📉 »</b> {download.eta()}\n"
                f"<b>│ Elapsed 📈 »</b> {elapsed}\n"
                f"<b>│ Engine 🛵 »</b> {escape(engine_txt)}\n"
            )
            if in_mode:
                msg += f"<b>│ In Mode 🤓 »</b> <code>#{escape(in_mode)}</code>\n"

            try:
                out_mode = download.upload_details.get('out_mode') or download.upload_details.get('upload_mode') or download.upload_details.get('type') or ''
            except Exception:
                out_mode = ''
            if out_mode:
                msg += f"<b>│ Out Mode 😊 »</b> <code>#{escape(str(out_mode))}</code>\n"

            if msg_link:
                msg += f"<b>│ Link 🔗 »</b> {msg_link}\n"

        elif status_txt == MirrorStatus.STATUS_SEEDING:
            msg += (
                f"<b>│ Status 🌱 »</b> <i>{escape(status_txt)}</i>\n"
                f"<b>│ Total 😲 »</b> {total_txt}\n"
                f"<b>│ Speed 🚀 »</b> {download.upload_speed()}\n"
                f"<b>│ Uploaded 📤 »</b> {download.uploaded_bytes()}\n"
                f"<b>│ Ratio 📊 »</b> {download.ratio()}\n"
                f"<b>│ Time 🕒 »</b> {download.seeding_time()}\n"
                f"<b>│ Engine 🛵 »</b> {escape(engine_txt)}\n"
            )
            if msg_link:
                msg += f"<b>│ Link 🔗 »</b> {msg_link}\n"
    ...
    if tasks > STATUS_LIMIT:
        ...
        buttons.ibutton("➡️ Next", "status nex")

...
_RDX_QUAL = ["WEB-DL", "WEBDL", "WEB DL", "WEBRip", "WEB RIP", "BluRay", "HDRip", "DVDRip", "CAM", "10bit", "8bit"]
...
def _rdx_parse_fields(raw_filename: str) -> dict:
    ...
    text = re_sub(r"\bWEB\s*[-_. ]*DL\b", "WEB-DL", text, flags=IGNORECASE)
    text = re_sub(r"\bWEB\s*[-_. ]*RIP\b", "WEBRip", text, flags=IGNORECASE)
    # remove common site/release tags that should never be part of {name}
    text = re_sub(r"\b(vegamovies|moviesmod|mkvcinemas|hdhub4u|hot)\b", " ", text, flags=IGNORECASE)
    ...
    # Quality / OTT / Lib
    bitdepth = ""
    mbit = re_search(r"\b(10|8)bit\b", text, IGNORECASE)
    if mbit:
        bitdepth = mbit.group(0)
    ...
    if bitdepth and (not quality or bitdepth.lower() not in quality.lower()):
        quality = (bitdepth + (" " + quality if quality else "")).strip()
